...
import { useRef, useSyncExternalStore, useCallback, memo, useMemo } from 'react'
...
const TRUST_ITEMS = ['50+ Projects','20+ Clients','3 Years','Bikaner, Rajasthan'] as const

// Optimized media query hook with memoized subscribe function
const mediaQuerySubscribe = (callback: () => void) => {
  const mq = window.matchMedia('(min-width: 1024px)')
  mq.addEventListener('change', callback)
  return () => mq.removeEventListener('change', callback)
}
const getMediaQuerySnapshot = () => window.matchMedia('(min-width: 1024px)').matches
const getMediaQueryServerSnapshot = () => false

// Memoized static components to prevent re-renders
const MicroLabel = memo(function MicroLabel() {
  return (
    <motion.div
...
const BodyText = memo(function BodyText() {
  return (
    <motion.p
...
const MobileServiceBadges = memo(function MobileServiceBadges() {
  return (
...
// Static glow styles (never change, so define outside component)
const violetGlowStyle = {
...
export function HeroSection() {
  ...
  // Optimized: memoized subscribe function prevents recreation on every render
  ...
  // Memoized event handlers to prevent recreation on re-renders
  const handlePortfolioMouseEnter = useCallback((e: React.MouseEvent<HTMLAnchorElement>) => {
    const el = e.currentTarget
    el.style.borderBottomColor = 'var(--color-muted)'
    el.style.color = 'var(--color-foreground)'
  }, [])

  const handlePortfolioMouseLeave = useCallback((e: React.MouseEvent<HTMLAnchorElement>) => {
    const el = e.currentTarget
    el.style.borderBottomColor = 'transparent'
    el.style.color = 'var(--color-muted)'
  }, [])

  // Memoized parallax values - '0%' for non-desktop to avoid motion value overhead
  const bgYValue = isDesktop ? bgY : '0%'
  const txtYValue = isDesktop ? txtY : '0%'
  const glowYValue = isDesktop ? glowY : '0%'
