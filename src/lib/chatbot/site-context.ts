...
import { pricingPlans, globalMarketBenchmarks, costComparisonModels } from '@/lib/data/pricing'
import {
  agreementSummaries,
  policyDocuments,
  agreementApplicabilityMatrix,
} from '@/lib/data/legal'
import { researchMeta, researchSections, leadToDeliveryPipeline } from '@/lib/data/research'
...
function summarizeService(service: (typeof services)[number]) {
  const pricing = service.pricing
    .slice(0, 3)
    .map((plan) => `${plan.label}: ${plan.amount}${plan.period ? ` (${plan.period})` : ''}`)
    .join(' | ')
...
function summarizePricingGroup(group: (typeof pricingPlans)[number]) {
  return [
    ...
    `Plans: ${group.plans
      .slice(0, 3)
      .map((plan) => `${plan.name} ${plan.price}${plan.period ? ` ${plan.period}` : ''}`)
      .join(' | ')}`,
  ]
    .filter(Boolean)
    .join('\n')
...
function summarizeTeamMember(member: (typeof teamMembers)[number]) {
  return [
    ...
    member.expertise?.length ? `Expertise: ${member.expertise.join('; ')}` : null,
  ]
    .filter(Boolean)
    .join('\n')
}
...
function summarizePolicyDocument(policy: (typeof policyDocuments)[number]) {
  return [
    ...
    `Sections: ${policy.sections.slice(0, 3).map((section) => section.heading).join(' | ')}`,
  ]
    .filter(Boolean)
    .join('\n')
}

function summarizeApplicability(row: (typeof agreementApplicabilityMatrix)[number]) {
  return [
    `${row.service}`,
...
function summarizePipelineStage(stage: (typeof leadToDeliveryPipeline)[number]) {
  return [
...
function summarizeMarketBenchmark(benchmark: (typeof globalMarketBenchmarks)[number]) {
  return [
    `${benchmark.service} [${benchmark.slug}]`,
    `MTA advantage: ${clip(benchmark.mtaAdvantage, 180)}`,
    `Top regions: ${benchmark.regions.slice(0, 3).map((region) => `${region.region} ${region.lowRange}-${region.highRange}`).join(' | ')}`,
  ]
    .filter(Boolean)
    .join('\n')
}
...
function summarizeServicesPage() {
  return [
    ...
    `Spotlight services: ${services.slice(0, 4).map((service) => `${service.name} (${service.priceLabel})`).join(' | ')}`,
...
function summarizePricingPage() {
  return [
    ...
    `Pricing groups: ${pricingPlans.slice(0, 5).map((group) => `${group.service} (${group.badge})`).join(' | ')}`,
    `Benchmarks: ${globalMarketBenchmarks.slice(0, 4).map((benchmark) => benchmark.service).join(' | ')}`,
...
function summarizeBlogPage() {
  return [
    ...
    `Top posts: ${blogPosts.slice(0, 5).map((post) => `${post.title} [${post.category}]`).join(' | ')}`,
  ].join('\n')
}
...
function summarizePortfolioPage() {
  return [
    ...
    `Featured projects: ${projects.slice(0, 4).map((project) => `${project.title} (${project.status})`).join(' | ')}`,
  ].join('\n')
}
...
function summarizeLegalPage() {
  return [
    'Page: /legal',
    `Agreements: ${agreementSummaries.slice(0, 8).map((agreement) => `${agreement.code} ${agreement.name}`).join(' | ')}`,
...
function summarizeCurrentPage(pathname?: string, pageTitle?: string, pageDescription?: string) {
  const route = (pathname ?? '/').split('?')[0] || '/'
  const titleLine = getDisplayTitle(pageTitle) ? `Page title: ${getDisplayTitle(pageTitle)}` : null
  const descriptionLine = pageDescription ? `Page description: ${clip(pageDescription, 180)}` : null

  if (route === '/') return joinDefined([titleLine, descriptionLine, summarizeHomePage()])
  if (route === '/services') return joinDefined([titleLine, descriptionLine, summarizeServicesPage()])
  if (route === '/pricing') return joinDefined([titleLine, descriptionLine, summarizePricingPage()])
  if (route === '/blog') return joinDefined([titleLine, descriptionLine, summarizeBlogPage()])
  if (route === '/portfolio') return joinDefined([titleLine, descriptionLine, summarizePortfolioPage()])
  if (route === '/research') return joinDefined([titleLine, descriptionLine, summarizeResearchPage()])
  if (route === '/legal' || route.startsWith('/legal/')) return joinDefined([titleLine, descriptionLine, summarizeLegalPage()])
  if (route === '/contact') return joinDefined([titleLine, descriptionLine, summarizeContactPage()])
  if (route.startsWith('/services/')) return joinDefined([titleLine, descriptionLine, summarizeServicePage(route)])
  ...
  if (route === '/trust-center' || route.startsWith('/cybersecurity-policy') || route.startsWith('/cybersecurity-training')) {
    return joinDefined([
      titleLine,
      descriptionLine,
...
export function buildSiteKnowledge(input: ChatContextInput = {}) {
  return [
    ...
    '',
    'PRICING',
    pricingPlans.map((group) => `- ${summarizePricingGroup(group)}`).join('\n\n'),
    '',
    'PROJECTS',
    projects.slice(0, 5).map((project) => `- ${summarizeProject(project)}`).join('\n\n'),
    '',
    'BLOG',
    blogPosts.slice(0, 5).map((post) => `- ${summarizeBlogPost(post)}`).join('\n\n'),
    '',
    'TEAM',
    teamMembers.map((member) => `- ${summarizeTeamMember(member)}`).join('\n\n'),
    '',
    'TESTIMONIALS / PRODUCT UPDATES',
    testimonials.map((testimonial) => `- ${summarizeTestimonial(testimonial)}`).join('\n\n'),
    '',
    'LEGAL & COMPLIANCE',
    agreementSummaries.slice(0, 8).map((agreement) => `- ${summarizeAgreement(agreement)}`).join('\n\n'),
    '',
    'POLICY DOCUMENTS',
    policyDocuments.map((policy) => `- ${summarizePolicyDocument(policy)}`).join('\n\n'),
    '',
    'AGREEMENT APPLICABILITY',
    agreementApplicabilityMatrix.map((row) => `- ${summarizeApplicability(row)}`).join('\n\n'),
    '',
    'RESEARCH & DELIVERY PIPELINE',
    researchSections.map((section) => `- ${summarizeResearchSection(section)}`).join('\n\n'),
    '',
    'PIPELINE STAGES',
    leadToDeliveryPipeline.map((stage) => `- ${summarizePipelineStage(stage)}`).join('\n\n'),
    '',
    'MARKET BENCHMARKS',
    globalMarketBenchmarks.slice(0, 4).map((benchmark) => `- ${summarizeMarketBenchmark(benchmark)}`).join('\n\n'),
    '',
    'COST MODELS',
    costComparisonModels.map((model) => `- ${summarizeCostModel(model)}`).join('\n\n'),
    '',
    'LIVE PAGE CONTEXT',
    summarizeCurrentPage(input.pathname, input.pageTitle, input.pageDescription),
  ].join('\n')
}
...
export function buildLocalAssistantReply(input: ChatContextInput = {}, userMessage: string) {
  const route = (input.pathname ?? '/').split('?')[0] || '/'
  const message = userMessage.toLowerCase()
  const pageLine = getDisplayTitle(input.pageTitle) ? `Page: ${getDisplayTitle(input.pageTitle)}` : `Page: ${route}`
  const pageDescription = input.pageDescription ? `Context: ${clip(input.pageDescription, 180)}` : null
  const serviceSlug = route.startsWith('/services/') ? route.split('/')[2] : undefined
  ...
  if (message.includes('ai') || message.includes('automation') || message.includes('bot') || message.includes('chatbot')) {
  ...
  if (message.includes('security') || message.includes('cyber') || message.includes('audit') || message.includes('vapt')) {
  ...
  if (message.includes('legal') || message.includes('privacy') || message.includes('dpa') || message.includes('consent')) {
    return [
      pageLine,
      pageDescription,
      'MTA works with service-specific agreements, NDAs, DPAs, and compliance docs for data handling.',
      `Key agreements: ${agreementSummaries.slice(0, 4).map((item) => `${item.code} ${item.name}`).join(' | ')}`,
  ...
  if (message.includes('project') || message.includes('quote') || message.includes('contact') || message.includes('start')) {
    return [
      pageLine,
      pageDescription,
